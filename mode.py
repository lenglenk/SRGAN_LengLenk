...
def test_only(args):
    ...
    with torch.no_grad():
        for i, te_data in enumerate(loader):
            ...
            # Assuming `te_data` includes the full path to each image
            input_image_path = te_data['LR_path'][0]  # Confirm 'LR_path' is the correct key
            original_filename = Path(input_image_path).stem
        
            result_filename = f'./result/{original_filename}.png'
